...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (glfwGetKey(window, GLFW_KEY_SPACE) && !isButtonPressed) 
		{
			if (camera->GetOrthoEnabled())
			{
		...
		if (isRotating) {
			transform = glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 0, 1)) * glm::translate(glm::mat4(1.0f), glm::vec3(1.f, 0.f, 0.f));
			transform2 = glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 0, 1)) * glm::translate(glm::mat4(1.0f), glm::vec3(3.f, 0.f, 0.f));
		}
