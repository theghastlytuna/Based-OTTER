...
int main() {
	...
	if (loadScene) {
		...
		Guid kiryuTex = ResourceManager::CreateTexture("textures/kiryu.png");
		...
		MaterialInfo::Sptr kiryuMaterial = std::make_shared<MaterialInfo>();
		kiryuMaterial->Shader = scene->BaseShader;
		kiryuMaterial->Texture = ResourceManager::GetTexture(kiryuTex);
		kiryuMaterial->Shininess = 1.f;
		scene->Materials[kiryuMaterial->GetGUID()] = kiryuMaterial;
