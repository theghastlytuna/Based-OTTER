...
void SimpleCameraControl::Update(float deltaTime)
{
	if (mouseControl == true)
	{
		...
		if (_isMousePressed) {
			...
			input.x = -static_cast<float>(currentMousePos.x - _prevMousePos.x) * _mouseSensitivity.x;
			input.y = static_cast<float>(currentMousePos.y - _prevMousePos.y) * _mouseSensitivity.y;

			_prevMousePos = currentMousePos;

			if (GetGameObject()->GetPosition().y >= 6.5)
			{
				input.y = -0.5;
			}
			if (GetGameObject()->GetPosition().y <= -6.5)
			...
			if (GetGameObject()->GetPosition().x <= 0)
			{
				input.x = 0.5;
			}
			...
			if (GetGameObject()->GetPosition().y >= 7.5)
			{
				LOG_INFO("Player Out Of Bounds! Position Reset");

				GetGameObject()->SetPostion(glm::vec3(10.f, 0.f, 0.f));
			}
			if (GetGameObject()->GetPosition().y <= -7.5)
			{
				LOG_INFO("Player Out Of Bounds! Position Reset");
			...
			if (GetGameObject()->GetPosition().x <= -0.5)
			{
				LOG_INFO("Player Out Of Bounds! Position Reset");

				GetGameObject()->SetPostion(glm::vec3(10.f, 0.f, 0.f));
			}
			if (GetGameObject()->GetPosition().x >= 12.5)
			{
				LOG_INFO("Player Out Of Bounds! Position Reset");
	...
	else
	{
		...
		if (glfwGetKey(_window, GLFW_KEY_W) && GetGameObject()->GetPosition().y > -6.5) {
			input.y -= _moveSpeeds.x;
		}
		if (glfwGetKey(_window, GLFW_KEY_S) && GetGameObject()->GetPosition().y < 6.5) {
			input.y += _moveSpeeds.x;
		}
		if (glfwGetKey(_window, GLFW_KEY_A) && GetGameObject()->GetPosition().x < 0) {
			input.x += _moveSpeeds.x;
		}
		if (glfwGetKey(_window, GLFW_KEY_D) && GetGameObject()->GetPosition().x > -12) {
			input.x -= _moveSpeeds.x;
		}
