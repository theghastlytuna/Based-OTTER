	...
	if (viewport->Size.x > 0 && viewport->Size.y > 0) {
		...
		if (ImGui::BeginMenuBar())
		{
			...
			if (ImGui::Button("Toggle Camera")) {
				if (app.CurrentScene()->MainCamera == app.CurrentScene()->WorldCamera)
				{
					app.CurrentScene()->MainCamera = app.CurrentScene()->PlayerCamera;
					app.CurrentScene()->MainCamera2 = app.CurrentScene()->PlayerCamera2;
				}
				else
				{
					app.CurrentScene()->MainCamera = app.CurrentScene()->WorldCamera;
					app.CurrentScene()->MainCamera2 = app.CurrentScene()->WorldCamera;
				}
			}
